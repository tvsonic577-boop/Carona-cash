      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 sm:p-6 font-sans relative overflow-x-hidden selection:bg-emerald-200 selection:text-emerald-950 animate-fade-in" id="login-screen-root">
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 z-10 my-auto py-8">
          ...
          <div className="lg:col-span-5 flex flex-col justify-center space-y-6 text-center lg:text-left">
            ...
            {/* QUICK PRE-LOADED EVALUATION CREDENTIALS CODES */}
            <div className="bg-slate-900/80 p-5 rounded-2xl border border-slate-800 text-left space-y-3 max-w-md mx-auto lg:mx-0 shadow-xl backdrop-blur-md">
              <p className="text-[10px] text-emerald-400 uppercase tracking-widest font-black flex items-center gap-1.5">
                <Sparkles size={11} className="text-emerald-400 animate-pulse" />
                Acesso Demonstrativo Rápido (Clique para Entrar):
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                
                <button
                  onClick={() => {
                    const email = 'amanda.lima@gmail.com';
                    const userObj = users.find(u => u.email.toLowerCase() === email && u.tipo === 'CLIENTE');
                    const clientObj = clientes.find(c => c.userId === userObj?.id);
                    if (clientObj) setActiveClienteId(clientObj.id);
                    setLoggedInEmail(email);
                    setSessionRole('CLIENTE');
                    setActivePortal('CLIENTE');
                    setIsLoggedIn(true);
                    addNotification("Conectado como Passageira Amanda!", "success");
                  }}
                  className="p-2.5 rounded-lg bg-slate-800/60 hover:bg-emerald-950/40 border border-slate-700/60 hover:border-emerald-600/50 text-left text-slate-100 transition duration-150 cursor-pointer block"
                >
                  <span className="font-bold text-emerald-400 block mb-0.5">👥 Passageira: Amanda</span>
                  <span className="text-[10px] text-slate-400 font-mono">Senha: amanda</span>
                </button>

                <button
                  onClick={() => {
                    const email = 'roberto.taxi@gmail.com';
                    const userObj = users.find(u => u.email.toLowerCase() === email && u.tipo === 'MOTORISTA');
                    const driverObj = motoristas.find(m => m.userId === userObj?.id);
                    if (driverObj) {
                      setActiveMotoristaId(driverObj.id);
                    }
                    setLoggedInEmail(email);
                    setSessionRole('MOTORISTA');
                    setActivePortal('MOTORISTA');
                    setIsLoggedIn(true);
                    addNotification("Conectado como Motorista Roberto!", "success");
                  }}
                  className="p-2.5 rounded-lg bg-slate-800/60 hover:bg-emerald-950/40 border border-slate-700/60 hover:border-emerald-600/50 text-left text-slate-100 transition duration-150 cursor-pointer block"
                >
                  <span className="font-bold text-emerald-400 block mb-0.5">🚗 Motorista: Roberto (Ativo)</span>
                  <span className="text-[10px] text-slate-400 font-mono">Senha: roberto</span>
                </button>

                <button
                  onClick={() => {
                    const email = 'willian.lima@outlook.com';
                    const userObj = users.find(u => u.email.toLowerCase() === email && u.tipo === 'MOTORISTA');
                    const driverObj = motoristas.find(m => m.userId === userObj?.id);
                    if (driverObj) {
                      setActiveMotoristaId(driverObj.id);
                    ...
                    setActivePortal('MOTORISTA');
                    setIsLoggedIn(true);
                    addNotification("Conectado como Motorista Willian (Pendente)!", "info");
                  }}
                  className="p-2.5 rounded-lg bg-slate-800/60 hover:bg-emerald-950/40 border border-slate-700/60 hover:border-emerald-600/50 text-left text-slate-100 transition duration-150 cursor-pointer block"
                >
                  <span className="font-bold text-amber-400 block mb-0.5">🚗 Motorista: Willian (Pend)</span>
                  <span className="text-[10px] text-slate-400 font-mono">Senha: willian</span>
                </button>
                ...
                <button
                  onClick={() => {
                    const email = 'tvsonic577@gmail.com';
                    setLoggedInEmail(email);
                    setSessionRole('ADMIN');
                    setActivePortal('ADMIN');
                    setIsLoggedIn(true);
                    addNotification("Conectado como Administrador Central!", "success");
                  }}
                  className="p-2.5 rounded-lg bg-slate-800/60 hover:bg-emerald-950/40 border border-slate-700/60 hover:border-emerald-600/50 text-left text-slate-100 transition duration-150 cursor-pointer col-span-1 sm:col-span-2 block text-center"
                >
                  <span className="font-bold text-amber-400 block mb-0.5">👑 Administrador Central Master</span>
                  <span className="text-[10px] text-slate-400 font-mono">E-mail: tvsonic577@gmail.com • Senha: Jr990387</span>
                </button>
